#!/usr/bin/env bash
#
# Batch wrapper: annotates every SAMPLE_autocycler/autocycler_out/consensus_assembly.fasta
# with Bakta (anno_env conda environment), writing to SAMPLE_autocycler/bakta/.
#
# Uses --keep-contig-headers so the GFF3 seqids stay exactly "1", "2", "3", ...
# matching Autocycler's own contig IDs — this is what lets
...
# the very short junk contigs Autocycler's multi-assembler consensus
# sometimes leaves in (a handful of bp long). The GFF3/TSV/etc. are already
# fully written by that point regardless — this only skips the cosmetic plot
# image, which identify_linear_plasmids.py doesn't use anyway.
#
# Usage:
