...
| 2d | `detect_boundary_clip_signature` | Detect soft/hard-clip pile-up exactly at contig ends via BAM (circular-seam artefact — the counterpart to 2c's depth-drop check). Scored only on short-read (Illumina) BAMs (`scoring_eligible`); sensitivity validated 2026-08 on pELF1/pELF2 (ratio 0.0-0.34, well under threshold), but specificity against a confirmed-circular Illumina sample is not yet validated — the 6 negative controls only have long-read data staged, see function docstring |
