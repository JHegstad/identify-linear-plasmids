...
# evidence set on every sample under an Autocycler-shaped output root, where
# each sample is its own SAMPLE_<suffix>/ directory containing:
#   SAMPLE_<suffix>/SAMPLE.fastq.gz                    (raw ONT long reads)
#   SAMPLE_<suffix>/autocycler_out/consensus_assembly.fasta
#   SAMPLE_<suffix>/autocycler_out/consensus_assembly.gfa
#
# SUFFIX (3rd arg) selects which run to process — e.g. "autocycler" for the
# full nine-assembler ensemble, or "bcapa" for a reduced Flye+Plassembler-
# only ensemble run through the same Autocycler consensus step. Both land
# in the same consensus_assembly.{fasta,gfa} shape, just under differently-
# suffixed sample directories, so one wrapper serves both.
#
# Uses SAMPLE_<suffix>/bakta/SAMPLE.gff3 for --annot when present (see
# run_bakta_autocycler_batch.sh — annotates consensus_assembly.fasta
# directly with --keep-contig-headers, so its seqids are the consensus
# assembly's own numeric contig IDs and match cleanly). Earlier revisions
# of this batch deliberately ran without --annot: the only Bakta
# annotations available at the time were run against the *Hybracter*
# assembly's contig IDs (chromosome00001, plasmid00001, ...), and since
# screen_genes() matches contig IDs by substring, feeding those in against
# numeric IDs would have risked spurious matches (contig "1" substring-
# matches "chromosome00001") rather than a clean skip.
#
# Usage:
#   ./run_autocycler_batch.sh [SRC_DIR] [THREADS] [SUFFIX]
#
# Outputs land in SAMPLE_<suffix>/linear_plasmid/SAMPLE.{tsv,json,...}.
